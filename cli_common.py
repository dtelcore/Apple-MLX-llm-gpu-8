...
def add_generate_decode_args(parser: argparse.ArgumentParser) -> None:
    """KV-cache / CUDA-graph flags used by generate.py and auto_train smoke generate."""
    parser.add_argument(
        "--no-kv-cache",
        action="store_true",
...
def add_quality_args(parser: argparse.ArgumentParser) -> None:
    ...
    group.add_argument(
        ...
        help="Comma weights spelling=1,punctuation=1,grammar=1,semantics=1 "
             "(chat mode also: turn_format, instruction_follow, coherence)",
    )
    group.add_argument(
        "--quality-mode", type=str, default=None, choices=["story", "chat", "both"],
        help="Quality scoring: story (default), chat, or both. "
             "chat_5m runs default to chat when omitted.",
    )
    group.add_argument(
...
def _prompt_quality_group(args: argparse.Namespace) -> None:
    ...
    if run_trial:
        ...
        def _as_quality_mode(raw: str) -> str:
            val = raw.strip().lower()
            if val not in ("story", "chat", "both"):
                raise ValueError(raw)
            return val
        _ask_value(
            args, "quality_mode", "Quality mode (story/chat/both)",
            _as_quality_mode, getattr(args, "quality_mode", None) or "story",
        )
    ...
    *,
    ...
    if entry == "generate":
        ...
        if entry == "train":
            ...
            groups.append((
                "smoke", "Smoke generate",
                "prompt=the, tokens=80, temp=0.6, top-k=10, top-p=0.9",
            ))
            groups.append(("decode", "Decode", "KV cache on, cuda-graph off"))
