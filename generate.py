...
Each invocation writes a dedicated file under output/logs/:
    generate_<checkpoint-stem>_<YYYYMMDD_HHMMSS>.log
(also appended to output/logs/training.log). The same per-run file is used when
this generate() path is called from auto_train.py after training.
"""
...
def generate(args: argparse.Namespace) -> str:
    ...
    source = getattr(args, "_entry", None) or "generate"
    logger.info(
        "generate run | source=%s | checkpoint=%s | prompt=%r | temp=%s | "
        "top_k=%s | top_p=%s | log=%s",
        source, args.checkpoint, args.prompt, args.temperature, args.top_k, args.top_p,
        log_path,
    )
