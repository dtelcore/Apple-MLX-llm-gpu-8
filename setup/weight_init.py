...
class WeightInitializer:
    ...
    def layer_init_scale(layer_type: str, fan_in: int, fan_out: int, 
        ...
        if fan_in is None or fan_in <= 0:
            raise ValueError(
                f"layer_init_scale({layer_type!r}): fan_in must be a positive int, got {fan_in!r}"
            )
...
def get_init_scales_for_config(config: Dict) -> Dict[str, float]:
    ...
    vocab_size = config.get('vocab_size')  # may be None until tokenizer is built
    ...
    # When vocab_size is deferred (BPE/char built at train start), fan_in falls
    # back to embedding_dim so interactive setup can compute scales before the
    # tokenizer exists. lm_head fan_in is already C and does not need vocab.
    embed_fan_in = vocab_size if vocab_size is not None else C
    if vocab_size is None:
        logger.info(
            "vocab_size deferred; token_embedding init scale uses embedding_dim=%s",
            C,
        )
    scales['token_embedding'] = WeightInitializer.layer_init_scale(
        'embedding', embed_fan_in, C
    )
    ...
    # Output projection (lm_head) [C, vocab_size] — fan_in is C; fan_out unused
    scales['lm_head'] = WeightInitializer.layer_init_scale(
        'lm_head', C, vocab_size if vocab_size is not None else C
    )
