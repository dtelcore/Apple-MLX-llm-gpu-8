...
from setup.dataset_setup import COMBINED_DATASET_NAME, DatasetLoader, DatasetAnalyzer, load_dataset_interactive, recommend_dataset_for_config
from setup.weight_init import setup_model_initialization, get_init_scales_for_config
from setup.training_presets import SCALE_PRESETS, apply_scale_preset, prompt_scale_preset
...
class TrainingSetup:
    ...
    def run_interactive_setup(self, use_presets: bool = True) -> Dict:
        ...
        if self.scale_preset in SCALE_PRESETS:
    ...
    def _setup_model_config(self, use_presets: bool = True):
        ...
        if use_presets:
            print("\nModel presets:")
            print("  toy / story_sub1m / tiny_stories / chat_5m / small / medium / custom")
            choice = input("\nUse preset or custom? [default=toy]: ").strip().lower()
            
            if choice in ['toy', 'tiny_stories', 'story_sub1m', 'chat_5m', 'tiny', 'small', 'medium']:
    ...
    def get_complete_config(self) -> Dict:
        ...
        if self.dataset_name == COMBINED_DATASET_NAME:
            dataset['combine'] = True
