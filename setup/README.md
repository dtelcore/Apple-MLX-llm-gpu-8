...
| **`story_sub1m`** | C=128, 8 heads, L=4, T=128, rmsnorm, RoPE, tied | LR **5e-4**, warmup **500**, batch **8**, accum **2**, cosine `min_lr_ratio=0.1`, **`window_stride=64`**, dropout **0** | combine all `data/*.txt` |
...
| **`chat_5m`** | C=256, 8 heads, L=6, T=128, rmsnorm, RoPE, tied | Same recipe as TinyStories; ~5M params | combine all `data/*.txt` |
...
model_config, hyperparams, dataset_name = apply_scale_preset("story_sub1m", vocab_size=256)
```
...
├── training_presets.py        # Scaling presets (toy / story_sub1m / tiny_stories / chat_5m)
├── story_sub1m_config.json    # Ready-to-run Fast Stories (<1M) config
├── chat_5m_config.json        # Ready-to-run Chat 5M (~5M) config
...
| **Fast Stories** | Auto | 128 | 8 | 4 | 128 | ~0.83M | (see `estimate_vram_footprint`) |
| **Tiny Stories** | Auto | 256 | 8 | 4 | 128 | ~3M | (see `estimate_vram_footprint`) |
| **Chat 5M** | Auto | 256 | 8 | 6 | 128 | ~5M | (see `estimate_vram_footprint`) |
