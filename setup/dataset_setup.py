...
class DatasetLoader:
    ...
    def load_by_name(self, dataset_name: str) -> List[str]:
        """Load a dataset by built-in name, combined data dir, or discovered file stem."""
        if dataset_name == COMBINED_DATASET_NAME:
            return self.load_combined_directory(self.data_dir)
    ...
    def load_combined_directory(self, directory: str = None,
                                pattern: str = '*.txt') -> List[str]:
        """Load every matching .txt file and concatenate non-empty lines.

        Files are read in sorted path order so the combined corpus is deterministic.
        Blank / whitespace-only lines are skipped. Empty files are skipped with a
        warning; the load fails if the directory is missing, has no matching files,
        or yields no documents.

        Args:
            directory: Folder to scan (default: this loader's data_dir)
            pattern: Glob pattern (default: '*.txt', non-recursive)

        Returns:
            Combined corpus (list of document strings)
        """
        data_dir = Path(directory or self.data_dir)
        if not data_dir.exists():
            raise FileNotFoundError(
                f"Data directory not found: {data_dir}. "
                "Create it and add one or more .txt files (one document per line)."
            )
        ...
        if not files:
            raise FileNotFoundError(
                ...
                "Add story text files (one document per line) before training."
            )
        ...
        for filepath in files:
            ...
            with open(filepath, 'r', encoding='utf-8', errors='replace') as handle:
                for raw_line in tqdm(handle, desc=f"Loading corpus: {basename}", unit="line"):
                    ...
                    if line:
                        corpus.append(line)
            n_added = len(corpus) - n_before
            if n_added:
                loaded_files.append(str(filepath))
                logger.info(f"[OK] {n_added:,} documents from {filepath}")
            else:
                logger.warning(f"Skipping empty file: {filepath}")

        if not corpus:
            raise ValueError(
                f"No text content found in {len(files)} {pattern} file(s) under {data_dir}."
            )
        ...
        self.datasets[name] = {
            ...
            'info': {
                'name': name,
                'description': f'Combined {len(loaded_files)} file(s) from {data_dir}',
                'files': loaded_files,
            },
        }
        self.current_dataset = name
        logger.info(
            f"[OK] Combined {len(corpus):,} documents from {len(loaded_files)} "
            f"file(s) in {data_dir}"
        )
        return corpus
    ...
    def list_datasets(self) -> Dict[str, Dict]:
        ...
        for name, filepath in self._discovered.items():
            if name == COMBINED_DATASET_NAME:
                continue  # reserved: combined directory mode, not a single file
    ...
    def interactive_select(self) -> List[str]:
        ...
        for i, name in enumerate(dataset_list, 1):
            ...
            if info['type'] == 'combined':
                print(f"  {i}. {name:24s} (combine {info['file_count']} file(s) in {info['source']})")
            elif info['type'] == 'discovered':
        ...
        while True:
            try:
                ...
                if 1 <= choice <= len(dataset_list):
                    ...
                    # Load if needed (covers combined data-dir, builtins not yet
                    # materialized, and files that were only discovered, not yet read)
                    if info['type'] == 'combined':
                        corpus = self.load_combined_directory(self.data_dir)
                    elif info['type'] == 'builtin' and selected_name not in self.datasets:
...
def recommend_dataset_for_config(model_config: Dict) -> str:
    ...
    if 'chat' in name or (embedding_dim >= 256 and num_layers >= 6):
